...
export default function AuthPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (success) {
      const user = demoAccounts.find(u => u.email === formData.email);
      if (user) {
        const config = roleConfig[user.role];
        router.push(config.redirect);
      }
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-violet-600 via-pink-500 to-orange-400 items-center justify-center p-12">
        <div className="max-w-md text-white">
          ...
          <h2 className="text-4xl font-bold mb-6">
            {mode === "signup" ? "Start Your Creator Journey" : "Welcome Back"}
          </h2>
          <p className="text-xl text-white/90 mb-8">
            {mode === "signup" 
              ? "Join 10,000+ creators using AI to grow their brand and land better deals."
              : "Continue your journey with AI-powered tools for creators, brands, and renters."
            }
          </p>
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-col items-center gap-1 p-3 rounded-xl text-xs font-medium transition-all ${
                          selectedRole === role
                            ? `${config.bgColor} ring-2 ring-offset-2 ring-${role === "gear_owner" ? "orange" : role === "renter" ? "teal" : role === "admin" ? "red" : role === "company" ? "blue" : "pink"}-500`
