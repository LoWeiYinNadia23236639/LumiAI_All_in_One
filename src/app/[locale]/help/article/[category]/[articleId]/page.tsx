...
import { articlesData as articlesDataEn } from "./data";
import { articlesData as articlesDataZh } from "./data.zh";
import { articlesData as articlesDataZhCn } from "./data.zh-cn";
import { notFound } from "next/navigation";
import { useLocale, useTranslations } from "next-intl";

const articlesDataByLocale: Record<string, typeof articlesDataEn> = {
  en: articlesDataEn,
  "zh-hk": articlesDataZh,
  "zh-cn": articlesDataZhCn,
};
...
export default function ArticlePage() {
  ...
  const articlesData = articlesDataByLocale[locale] ?? articlesDataEn;
  ...
  const categoryKeyMap: Record<string, { titleKey: string; color: string }> = {
    "getting-started": { titleKey: "gettingStarted", color: "violet" },
    "ai-tools": { titleKey: "aiTools", color: "pink" },
    marketplace: { titleKey: "brandMarketplace", color: "orange" },
    equipment: { titleKey: "equipmentRental", color: "blue" },
    analytics: { titleKey: "analyticsHelp", color: "green" },
  };

  const currentCatInfo = categoryKeyMap[category] || { titleKey: "helpCenter", color: "violet" };
  const currentCatTitle = t(currentCatInfo.titleKey as any);
  ...
  const colors = colorClasses[currentCatInfo.color];
    ...
    <main className="min-h-screen bg-gradient-to-b from-white via-gray-50/50 to-white pt-24 pb-16">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <nav className="flex items-center gap-2 text-sm text-neutral-500 mb-6">
          <Link href="/help" className="hover:text-violet-600 transition-colors">{t("helpCenter")}</Link>
          <ChevronRight className="w-4 h-4" />
          <span className={`${colors.text} font-medium`}>{currentCatTitle}</span>
          <ChevronRight className="w-4 h-4" />
        ...
        >
          ...
          <div className="flex items-center gap-4 text-sm text-neutral-500">
            ...
            <span>•</span>
            <span>{t("lastUpdated")}: {article.lastUpdated}</span>
          </div>
        </motion.div>
        ...
        >
          ...
          {article.relatedArticles && article.relatedArticles.length > 0 && (
            <div className="border-t mt-8 pt-8">
              <h3 className="font-bold text-lg mb-4">{t("relatedArticles")}</h3>
        ...
        >
          <p className="text-neutral-600 mb-3">{t("wasThisHelpful")}</p>
